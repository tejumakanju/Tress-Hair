  ...
  return (
    <div className="fixed inset-0 z-[60] md:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
      <button
        ...
        className="absolute inset-0 bg-noir/45 animate-fade-in"
        onClick={onClose}
      />
      <aside className="absolute left-0 top-0 bottom-0 w-[min(100%,22rem)] bg-ivory shadow-2xl flex flex-col animate-slide-in-left safe-pt safe-pb">
        ...
        <nav className="flex-1 overflow-y-auto px-5 py-2 overscroll-contain">
          {mainNavigation.map((item) => (
            <MobileNavSection key={item.label} item={item} onNavigate={onClose} />
          ))}
        </nav>

        <div className="border-t border-border px-5 py-4 space-y-3 bg-cream/40">
          ...
          <Link
            href="/account"
            onClick={onClose}
            className="flex items-center gap-3 min-h-11 py-2.5 text-xs tracking-[0.12em] uppercase hover:text-champagne-dark transition-colors"
          >
